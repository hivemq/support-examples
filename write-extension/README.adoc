...
= HiveMQ 4 Hello Write Extension
...
This Hello Write extension sets a ClientLifecycleEventListener which attribute to the ConnectionAttributeStore.
...
. Use JDK 11 for Gradle and JDK 21 for the Java project.
. Execute the Gradle task `hivemqExtensionZip` to build the extension.
. Move the file: `build/hivemq-extension/hivemq-hello-write-extension-4.39.0.zip` to the directory: `HIVEMQ_HOME/extensions`
. Unzip the file.
. Start HiveMQ.
...
If you want to contribute to HiveMQ Hello Write Extension, see the link:CONTRIBUTING.md[contribution guidelines].

== License

HiveMQ Hello Write Extension is licensed under the `APACHE LICENSE, VERSION 2.0`.
