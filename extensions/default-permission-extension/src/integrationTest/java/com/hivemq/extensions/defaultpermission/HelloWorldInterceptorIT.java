...
/**
 * This tests the functionality of the {@link DefaultPermissionInterceptor}.
...
class DefaultPermissionInterceptorIT {
    ...
    void test_payload_modified() throws InterruptedException {
        ...
        client.subscribeWith().topicFilter("default/permission").send();

        client.publishWith().topic("default/world").payload("Good Bye Permission!".getBytes(StandardCharsets.UTF_8)).send();

        final Mqtt5Publish received = publishes.receive();
        assertEquals("Default Permission!", new String(received.getPayloadAsBytes(), StandardCharsets.UTF_8));
        publishes.close();
    }
}